...
- [x] **第一阶段：跑通盲测打分核心闭环**
优先落地豆包大模型的盲题生成、自动打分能力，实时同步全量匿名数据到公开看板，所有用户可以随时查看各流派的实时效力数据，先跑通最小验证逻辑。
- [x] **第二阶段：拓展测算池与场景覆盖**
逐步接入塔罗、奇门遁甲、星座运势等更多中西命理工具，补充互动性更强的测算形式，覆盖更细分的预测场景，扩大验证的样本范围和流派覆盖度，最终形成覆盖所有主流玄学方法的完整验证体系。
